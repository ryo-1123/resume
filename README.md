...
- 監視設定
  - AWS上のインフラとアプリケーションの監視設定を確立、異常検知とアラート通知を自動化
- ドキュメンテーション
...
- インフラ設計と構築
  ...
  - インフラストラクチャアズコード(IaC)を用いて、リソースのプロビジョニングと管理を自動化し、迅速なデプロイメントと一貫した環境を確保
  - Lambdaとその他のAWSサービスの組み合わせにより、リソースの使用効率を最大化
- 運用と監視
  - CloudWatchとカスタムスクリプトを組み合わせて、システムとアプリケーションの監視設定を最適化
  - アラートとログ管理を強化し、異常検知とインシデントレスポンスの速度をXX%向上させた
- 開発者へのナレッジトランスファー
...
- その他: Gitlab CI
​
#### **3. AWSリソースの暗号化・クロスアカウント管理のバックアップ基盤構築**
##### **職務内容**
AWSクラウド基盤上での本番運用の効率とセキュリティを向上させるためのインフラストラクチャの改善と最適化に取り組む
##### **主な実績**
- AWSリソースのセキュリティ強化
  - 本番運用のセキュリティ要件を満たすため、AWSリソースの暗号化を計画・実行。KMS(Key Management Service)を使用して、データの暗号化とキー管理を効率的に行った。
  - 暗号化のベストプラクティスを適用し、データの保護とプライバシーの確保を強化。セキュリティコンプライアンスの遵守を確保した。
- クロスアカウントバックアップ基盤の構築
  - AWSのクロスアカウント管理策を採用して、バックアップとディザスタリカバリ基盤を効率的に構築・運用。
  - AWS OrganizationsとIAMロールの最適な設定を用いて、複数アカウント間でのリソース管理とデータのアクセス制御を強化。
##### **使用技術**
- クラウドプラットフォーム: AWS(KMS, Backup)
- IaC: Cloudformation
...
- 開発者体験の向上
  - ローカル開発環境のセットアップと管理の効率化を目指して、コンテナ技術と自動化スクリプトを導入。
  - CI/CDパイプラインを最適化し、ビルドとデプロイの時間をXX%短縮。開発者の生産性を向上させた。
  - Gitlab CIの実行コスト削減
  - 単純作業の自動化
  - デプロイフローの改善(Ruby, Shell)
...
### **Ficilcom Inc.(副業)**
Infrastructure・Backend Engineer / February 2023
​
...
## **Activities | 課外活動**
### **Webサービスの新規開発 サントネール株式会社  April 2022 ~**
